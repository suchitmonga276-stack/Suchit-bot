...
// **Bot Maintenance & Status Middleware**
...
// **Start Command & Force Subscription**
bot.start(async (ctx) => {
  ...
  return ctx.reply(db.settings.startText, {
    parse_mode: 'Markdown',
    reply_markup: {
      keyboard: [
...
bot.action('check_sub', async (ctx) => {
  ...
  return ctx.reply("✅ **Thank you! Send /start again to open the bot menu.**", { parse_mode: 'Markdown' });
});
...
bot.command('setstart', (ctx) => {
  ...
  if (!newText) return ctx.reply("❌ **Please provide text!**", { parse_mode: 'Markdown' });
...
bot.command('addchannel', (ctx) => {
  ...
  if (!ch) return ctx.reply("❌ **Provide channel username!**", { parse_mode: 'Markdown' });
...
bot.command('gift', (ctx) => {
  ...
  delete db.giftCodes[code];

  ctx.reply(`🎉 **Gift Code Claimed Successfully!**\n💳 **Added:** ₹${amt}`, { parse_mode: 'Markdown' });
});

bot.launch();
console.log('Bot is running smoothly without crashing...');
